...
func fakeDataReader(t *testing.T, compress, multi bool) (string, *Reader) {
	...
	if multi {
		err = GenerateMockMultiHfile(f.Name(), 100000, 1024*4, compress, false)
		assert.Nil(t, err, "cannot write to tempfile: ", err)
	} else {
		err = GenerateMockHfile(f.Name(), 100000, 1024*4, compress, false)
		assert.Nil(t, err, "cannot write to tempfile: ", err)
	}
...
func TestFirstKeys(t *testing.T) {
	f, err := os.Open("testdata/pairs.hfile")
	if err != nil {
		t.Fatal(err)
	}
	r, err := NewReader(f)
